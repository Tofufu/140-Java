...
// QUESTION 1: What is purpose of putback?
            ...
            /*          putback = boolean s.t. it's set true when we get something as a character. if we haven't gotten anything we're going to get another character and set putback as false
                it's used as a checker to see if we have a character within our scanner*/
...
//             'hello' is returned from the input 'hello*45'?
            ...
            if( putback) {              //if putback true then set it back to false
            putback = false;
            }
            else {                      //get next token to look at
                c = getchar();
            }
            
            
            if ( myisalpha((char) c) ) { //it if it's a letter
                /* identifier. */
                String id = buildID();
                return new Token(keywordLookup(id), id, linenumber);        //lookup token in the syntax tree
            }
            else if ( myisdigit((char) c) ) {   // if it's a number look it up via number syntax tree
                /* number. */
                return new Token(TK.NUM, buildNUM(), linenumber);
            }
            else {
                
                //otherwise if it's not a letter and it's a special case
                switch( c ) {
                    case '(':
...
// QUESTION 3:  What does the following case and the code in it do?
                    ...
                    case '-':
                        //CHANGED HERE, ADDED THE MINUS CASE INSIDE TK.JAVA
                        return ccase1or2('-','>',TK.MINUS,TK.ARROW);
                    case '+':
                        return ccase1('+', TK.PLUS);
                    //CHANGED HERE ADDED THE +
                    case EOF:
                        got_eof = true;
                    ...
                    case '#': // gobble comments
                        ...
                        putback = true; //set true after each time we get another character
                        break;
                    default:
                        System.err.println(
